...
public class VillageMapGenMod extends MapGenVillage {
    /** A list of all the biomes villages can spawn in. */
    private static List<Biome> VILLAGE_SPAWN_BIOMES = Arrays.<Biome>asList(Biomes.PLAINS, Biomes.DESERT, Biomes.SAVANNA, Biomes.TAIGA, Biomes.BEACH);
    /** None */
    private int size;
    private int distance;
    private final int minTownSeparation;

    public VillageMapGenMod()
    {
        this.distance = 32;
        this.minTownSeparation = 8;
    }

    public VillageMapGenMod(Map<String, String> map)
    {
        this();
    ...
    {
        return new VillageMapGenMod.Start(this.world, this.rand, chunkX, chunkZ, this.size);
    }
    ...
    public static class Start extends StructureStart
    {
        ...
        {
            super(x, z);
            List<VillageMod.PieceWeight> list = VillageMod.getStructureVillageWeightedPieceList(rand, size);
            VillageMod.Start structurevillagepieces$start = new VillageMod.Start(worldIn.getBiomeProvider(), 0, rand, (x << 4) + 2, (z << 4) + 2, list, size);
